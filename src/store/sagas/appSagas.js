...
function* getBlogs(params) {
  console.log(params, 'params in blogs Api');
  try {
    const config = {
      url: urls.getBlogByUser,
      method: 'GET',
      headers: {
        Authorization: `Bearer ${params && params.payload}`,
      },
    ...
    const response = yield request(config);
    console.log(config, 'senidg to ');
    console.log(response, '<<<<<<<< blogs  response  >>>>>>>>>>>>>>>>>');

    if (response?.data?.response?.isSuccess) {
      yield put({
        type: actionTypes.GET_BLOGS_SUCCEEDED,
      });
      params.cb(response);
    } else {
      yield put({
...
function* getBlogCatagorySaga(params) {
  console.log(params, 'params in blogs Api');
  try {
    const config = {
      url: urls.blogCatagory,
      method: 'GET',
      headers: {
        Authorization: `Bearer ${params && params.payload}`,
      },
    ...
    console.log(
      response,
      '<<<<<<<< blogs  category response  >>>>>>>>>>>>>>>>>',
    );

    if (response?.data?.response?.isSuccess) {
      yield put({
        type: actionTypes.GET_BLOG_CATEGORY_SUCCEEDED,
      });
      params.cb(response);
    } else {
      yield put({
        type: actionTypes.GET_BLOG_CATEGORY_FAIL,
      });
    }
  } catch (error) {
    console.log(error, 'err on 1189');
...
function* createBlogByUserSaga(params) {
  console.log(params, 'params in create blog');
  try {
    const config = {
      url: urls.createBlogByUser,
      method: 'POST',
      data: params && params.params,
      // headers: {
      //   Authorization: `Bearer ${params && params.cb}`,
    ...
    if (response && response.data && response.data.response) {
      yield put({
        type: actionTypes.CREATE_BLOGS_SUCCEEDED,
      });
      showSuccessAlert(response?.data?.response?.message);
      NavigationService.goBack();
    } else {
      showErrorAlert(response?.data?.message);
      yield put({
        type: actionTypes.CREATE_BLOGS_FAIL,
      });
    }
  } catch (error) {
    showErrorAlert(getAPIError(error));
