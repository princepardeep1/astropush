...
export default function* rootSaga() {
  ...
  yield takeLatest(actionTypes.VERIFY_EMAIL_REQUESTED, verifyLogin);
  ...
  yield takeLatest(
    actionTypes.UPDATE_PROFILE_INFO_REQUESTED,
    editProfileInfosaga,
  );

  // //app sagas
  yield takeLatest(actionTypes.GET_BLOGS_REQUESTED, getBlogs);
  yield takeLatest(actionTypes.CREATE_BLOGS_REQUESTED, createBlogByUserSaga);
  yield takeLatest(
    actionTypes.GET_BLOG_CATEGORY_REQUESTED,
    getBlogCatagorySaga,
  );
  ...
  yield takeLatest(
    actionTypes.SAVE_PHOTO_SHARING_REQUESTED,
    savephotosharingsaga,
  );
  yield takeLatest(actionTypes.TIMELINE_LIST_REQUESTED, gettimeline);
  yield takeLatest(
    actionTypes.PHOTOSHARE_ADDLIKE_REQUESTED,
    addLikeInphotoshareSaga,
  );
  yield takeLatest(
    actionTypes.PHOTO_SHARE_LIKES_REQUESTED,
    likesphotosharinglist,
  );
  yield takeLatest(
    actionTypes.PHOTO_ADDCOMMENT_REQUESTED,
    addcommentphotodharingsaga,
  );
  yield takeLatest(
    actionTypes.PHOTOSHARE_COMMENT_LIST_REQUESTED,
    commentListphotoharing,
  );
  yield takeLatest(
    actionTypes.LEADERBOARD_FISH_LIST_REQUESTED,
    getleaderboardfishlist,
  );
  yield takeLatest(
    actionTypes.LEADERBOARD_RANKING_REQUESTED,
    leaderboardrankingsaga,
  );
  yield takeLatest(
    actionTypes.LEADERBOARD_FILTER_REQUESTED,
    leaderboardfiltersaga,
  );
  yield takeLatest(
    actionTypes.TOURNAMENT_LISTING_REQUESTED,
    gettournamentlistingsaga,
  );
  yield takeLatest(
    actionTypes.TERMS_AND_CONDITION_REQUESTED,
    gettermsconditionsaga,
  );
  yield takeLatest(actionTypes.MEMBER_LISTING_REQUESTED, getmemberlist);
  yield takeLatest(
    actionTypes.SEND_FRIEND_REQUESTS_REQUESTED,
    sendfriendrequestsaga,
  );
  ...
  yield takeLatest(
    actionTypes.SAVE_SURVEY_QUESTION_REQUESTED,
    postquestioninarry,
  );
}
