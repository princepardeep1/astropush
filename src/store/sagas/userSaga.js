...
function* editProfileInfosaga(params) {
  ...
  try {
    const config = {
      url: urls.AstrologerUpdateProfile,
      method: 'POST',
      data: params?.params,
      headers: {
    ...
    const response = yield request(config);
    console.log(response, 'editProfilesaga api ');

    if (response && response.data && response.data.success) {
      yield put({
        type: actionTypes.UPDATE_PROFILE_INFO_SUCCEEDED,
      });
      showSuccessAlert(response.data.message);
    }
    ...
    yield put({
      type: actionTypes.UPDATE_PROFILE_INFO_FAIL,
    });
  }
}
